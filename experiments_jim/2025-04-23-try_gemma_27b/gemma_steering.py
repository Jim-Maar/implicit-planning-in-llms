...
def get_results(model, tokenizer, positive_prompts, negative_prompts, generation_prompt, positive_words, negative_words, layer=27, steering_multiplier=1.5, max_new_tokens=30):
    ...
    print("Steered:")
    print(text_steered[:10])
    print("Negsteered:")
    print(text_negsteered[:10])
    positive_words_fraction_baseline = get_last_word_fraction(text_unsteered, positive_words)
    ...
    negative_words_fraction_baseline = get_last_word_fraction(text_unsteered, negative_words)
...
layers = np.linspace(25, 35, 11).astype(int)
magnitudes = np.linspace(0.5, 2.0, 4)
rabbit_likelyhood = np.zeros((len(layers), len(magnitudes)))
habit_likelyhood = np.zeros((len(layers), len(magnitudes)))
rabbit_vs_habit_likelyhood = np.zeros((len(layers), len(magnitudes)))
for i, layer in enumerate(layers):
    print(f"Layer: {layer}")
    steering_vector = get_steering_vector_fast(model, tokenizer, POSITIVE_PROMPTS, NEGATIVE_PROMPTS, layer)
    for j, magnitude in enumerate(magnitudes):
        text_steered = generate_steered_output(steering_vector, model, tokenizer, GENERATION_PROMPT, 200, layer, magnitude)
        rabbit_likelyhood[i, j] = get_last_word_fraction(text_steered, positive_words)
        habit_likelyhood[i, j] = get_last_word_fraction(text_steered, negative_words)
        rabbit_vs_habit_likelyhood[i, j] = rabbit_likelyhood[i, j] - habit_likelyhood[i, j]
# %%
# plot heatmap with plotly
import plotly.express as px
fig = px.imshow(rabbit_likelyhood, x=magnitudes, y=layers, color_continuous_scale='Viridis')
fig.show()
fig = px.imshow(habit_likelyhood, x=magnitudes, y=layers, color_continuous_scale='Viridis')
fig.show()
# %%
...
STEERING_MULTIPLIER = 1.5
# ## 7. Try out different Layers and Magnitudes for steering
layers = list(range(0, 46))
rabbit_likelyhood = np.zeros((len(layers)))
habit_likelyhood = np.zeros((len(layers)))
rabbit_likelyhood_negsteered = np.zeros((len(layers)))
habit_likelyhood_negsteered = np.zeros((len(layers)))
for i, layer in enumerate(layers):
    print(f"Layer: {layer}")
    steering_vector = get_steering_vector_fast(model, tokenizer, POSITIVE_PROMPTS, NEGATIVE_PROMPTS, layer)
    text_steered = generate_steered_output(steering_vector, model, tokenizer, GENERATION_PROMPT, 500, layer, STEERING_MULTIPLIER)
    text_negsteered = generate_steered_output(-steering_vector, model, tokenizer, GENERATION_PROMPT, 500, layer, STEERING_MULTIPLIER)
    rabbit_likelyhood[i] = get_last_word_fraction(text_steered, positive_words)
    habit_likelyhood[i] = get_last_word_fraction(text_steered, negative_words)
    rabbit_likelyhood_negsteered[i] = get_last_word_fraction(text_negsteered, positive_words)
    habit_likelyhood_negsteered[i] = get_last_word_fraction(text_negsteered, negative_words)
# %%
# plotly line plot of rabbit_likelyhood, habit_likely_hood, rabbit_likelyhood_negsteered and habit_likelyhood_negsteered
# rabbit should be green and habit should be blue, steered should be solid and negsteered should be dotted
import plotly.graph_objects as go

fig = go.Figure()

# Add traces for rabbit likelihood (green)
...
fig.add_trace(go.Scatter(
    x=layers,
    y=rabbit_likelyhood_negsteered,
    mode='lines',
    name='Rabbit (Neg Steered)',
    line=dict(color='green', width=2, dash='dot')
))

# Add traces for habit likelihood (blue)
fig.add_trace(go.Scatter(
    x=layers,
    y=habit_likelyhood,
    mode='lines',
...
fig.add_trace(go.Scatter(
    x=layers,
    y=habit_likelyhood_negsteered,
    mode='lines',
    name='Habit (Neg Steered)',
    line=dict(color='blue', width=2, dash='dot')
))

# Update layout
fig.update_layout(
    title='Effect of Steering Across Different Layers',
    xaxis_title='Layer',
...
for i in tqdm(range(1, min(len(NEGATIVE_PROMPTS), len(POSITIVE_PROMPTS)), 2)):
...
x_values = list(range(1, min(len(NEGATIVE_PROMPTS), len(POSITIVE_PROMPTS)), 2))
